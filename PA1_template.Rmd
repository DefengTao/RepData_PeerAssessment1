...
medianb <- median(b[,2])
```
The mean is `r meanb` and the median is `r medianb`.
...
```
The 5-minute interval, on average across all the days in the dataset, which contains the maximum number of steps is `r number`.
...
```
The total number of missing value is `r row`. I use the mean for that 5-minute interval to fill in the missing value. As a result, the mean is `r meannewb` and the median is `r mediannewb`. The mean value doesn't change, because I use the mean value to fill the NA value, however, the median value is a little bit different than previous one. It is easy to understand that the size of the sample is different, so the median will slightly change.
